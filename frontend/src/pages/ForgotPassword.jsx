...
import { forgotPasswordEmail, forgotPasswordPhone, forgotPasswordVerify } from "../services/api";

export default function ForgotPassword() {
  const nav = useNavigate();
  const { success, error: showError } = useNotification();
  
  const [step, setStep] = useState(1); // 1: chọn phương thức, 2: nhập OTP, 3: nhập mật khẩu mới
  ...
  const sendOtp = async () => {
    setError("");
    if (!identifier) {
      const msg = method === "email" ? "Please enter your email" : "Please enter your phone number";
      setError(msg);
      showError(msg);
      return;
    }

    if (method === "email" && !identifier.endsWith("@gmail.com")) {
      const msg = "Email must be @gmail.com";
      setError(msg);
      showError(msg);
      return;
    }
    ...
    try {
      if (method === "email") {
        ...
        await forgotPasswordPhone(identifier);
      }
      setStep(2);
      setCountdown(60);
      setResendDisabled(true);
      success("OTP sent to your " + (method === "email" ? "email" : "phone") + "!");
    } catch (err) {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await forgotPasswordVerify(identifier, otpCode, "", method);
  ...
  const resetPassword = async () => {
    ...
    setLoading(true);
    try {
      await forgotPasswordVerify(identifier, otpCode, newPassword, method);
            ...
            <div className="relative rounded-3xl bg-white/80 backdrop-blur-xl border border-white/50 p-8 md:p-10 shadow-xl shadow-slate-200/50 animate-fade-in-up">
              ...
              <div className="text-center mb-8 relative">
                ...
                <p className="text-slate-600 text-sm">
                  ...
                  <span className="font-semibold text-green-600">{identifier}</span>
                </p>
              </div>
  ...
  // Step 1: Select method and enter identifier
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 text-slate-900 overflow-hidden font-sans">
      {/* Animated Background */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="w-full max-w-[440px]">
          <div className="relative rounded-3xl bg-white/80 backdrop-blur-xl border border-white/50 p-8 md:p-10 shadow-xl shadow-slate-200/50 animate-fade-in-up">
            ...
            <div className="text-center mb-8 relative">
              ...
              <p className="text-slate-600">Select a method to reset your password</p>
            </div>
            ...
            {/* Method Selection - Two big buttons */}
            <div className="grid grid-cols-2 gap-4 mb-8">
              <button
                onClick={() => { setMethod("phone"); setError(""); }}
                className={`p-6 rounded-2xl border-2 transition-all duration-300 ${
                  method === "phone"
                    ? "border-green-500 bg-green-50 shadow-lg shadow-green-600/20"
                    : "border-slate-200 bg-white hover:border-green-300 hover:bg-green-50/50"
                }`}
              >
                <div className="flex flex-col items-center gap-3">
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center ${
                    method === "phone" ? "bg-green-500 text-white" : "bg-slate-100 text-slate-600"
                  }`}>
                  ...
                  <div className="text-center">
                    <span className={`block font-semibold ${method === "phone" ? "text-green-600" : "text-slate-700"}`}>
                      SMS
                    </span>
                    <span className="text-xs text-slate-500">Gửi qua số điện thoại</span>
                  </div>
                </div>
              </button>
              ...
              >
                <div className="flex flex-col items-center gap-3">
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center ${
                    method === "email" ? "bg-green-500 text-white" : "bg-slate-100 text-slate-600"
                  }`}>
                    <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                  ...
                  <div className="text-center">
                    <span className={`block font-semibold ${method === "email" ? "text-green-600" : "text-slate-700"}`}>
                      Email
                    </span>
                    <span className="text-xs text-slate-500">Gửi qua địa chỉ email</span>
                  </div>
                </div>
              </button>
            </div>
              ...
              <div className="relative">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={method === "email" ? "your.email@gmail.com" : "0123456789"}
                  />
                </div>
              </div>
            </div>
