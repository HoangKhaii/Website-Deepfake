...
const ERROR_MESSAGES = {
  google_not_configured: "Google login is not configured. Please configure GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in the server's .env file.",
  google_auth_failed: "Google login failed. You can try again or log in using email.",
};
...
export default function Login() {
  ...
  const loginEmail = async () => {
    ...
    try {
      ...
      if (err?.body?.needsFaceRegistration === true) {
        const msg = err.message || "Face registration not confirmed. Please register again.";
        setError(msg);
        showError(msg);
        nav("/register");
        return;
  ...
  const loginFace = () => {
    ...
    if (!trimmedEmail) {
      const msg = "Please enter your email before using Face ID.";
      setError(msg);
      showError(msg);
      return;
    }
    // state + email trên query để F5/reload vẫn biết user (state mất sau reload)
    nav(`/face-scan?type=login-face&email=${encodeURIComponent(trimmedEmail)}`, {
            ...
            {mode === "face" && (
              <div className="text-center py-6">
                <div className="mb-5 text-left">
                  <label className="block text-slate-600 text-sm font-medium mb-2">Email for Face ID</label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                ...
                <p className="text-slate-600 mb-2">Use your face to sign in securely</p>
                <p className="text-slate-500 text-sm mb-4">Enter your email above, then tap Face ID.</p>
                <button
                  onClick={loginFace}
