...
async function start() {
  ...
  server = http.createServer(app).listen(PORT, '0.0.0.0', () => {
    ...
    const gwCompareUrl =
      process.env.AI_GATEWAY_COMPARE_URL ||
      'http://26.54.212.200:5001/api/compare-faces (default)';
    const antiSpoofUrl =
      process.env.ANTI_SPOOF_PREDICT_URL ||
      process.env.ANTILOGIN_GATEWAY_PREDICT_URL ||
      'http://26.54.212.200:5003/api/predict (default)';
    console.log(`🤖 anti-spoof (face login): ${antiSpoofUrl}`);
    console.log(`🤖 deepfake gateway (detect): ${gwUrl}`);
    console.log(`🤖 deepfake gateway (compare): ${gwCompareUrl}`);
