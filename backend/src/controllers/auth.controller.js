...
const { registerFace, verifyFace } = require('../services/faceAuth.service');
  ...
  try {
    ...
    if (user.status === 'pending_face') {
      return res.status(403).json({
        message: 'Your account has not been verified with facial recognition. Please complete the facial registration.',
...
// Gửi OTP cho đăng ký (KHÔNG tạo user DB ở bước này)
async function registerOtpSend(req, res) {
  try {
    ...
    const currentDraft = getPendingRegistration(normalizedEmail);
    if (currentDraft?.otpSentAt) {
      const secondsSinceLastOtp = (Date.now() - new Date(currentDraft.otpSentAt).getTime()) / 1000;
    ...
    setPendingRegistration(normalizedEmail, {
      ...
      otpCode,
      otpExpiredAt: expiredAt.toISOString(),
    ...
    console.log(`[Register OTP Draft] ${normalizedEmail} => ${otpCode} (expires ${expiredAt.toISOString()})`);
...
// Xác thực OTP cho bản đăng ký tạm (chưa lưu users)
async function registerOtpVerify(req, res) {
  try {
    ...
    if (!draft) {
      return res.status(401).json({ message: 'OTP not found. Please request OTP again.' });
    }
    if (draft.verified) {
      return res.status(400).json({ message: 'OTP already verified. Please continue face registration.' });
    }

    const submittedOtp = String(otp_code).trim();
    if (submittedOtp !== String(draft.otpCode || '')) {
      return res.status(401).json({ message: 'Invalid or expired OTP' });
    }

    const expiredAt = new Date(draft.otpExpiredAt);
    if (!(expiredAt instanceof Date) || Number.isNaN(expiredAt.getTime()) || expiredAt < new Date()) {
      return res.status(401).json({ message: 'Invalid or expired OTP' });
    }
    ...
    return res.json({
      ...
      message: 'OTP verified. Please complete face registration to activate your account.',
    });
  } catch (err) {
...
async function getMe(req, res) {
  try {
    if (!req.userId && req.userEmail) {
      const googlePending = getPendingGoogleProfile(req.userEmail);
      if (googlePending) {
        return res.json({
          user: {
            email: googlePending.email,
            full_name: googlePending.full_name || null,
            role: 'user',
            status: 'pending_face',
            pending_google: true,
            has_face: false,
    ...
    const { rows } = await query(
      `SELECT user_id, username, email, phone_number, full_name, role, status, created_at, last_login, face_image
       FROM users WHERE user_id = $1 LIMIT 1`,
      [req.userId]
    );
    const user = rows[0];
    if (!user) return res.status(404).json({ message: 'User not found' });
    const hasFace = Boolean(user.face_image);
    const safe = toSafeUser(user);
    delete safe.face_image;
    safe.has_face = hasFace;
...
// Xác minh OTP và đổi mật khẩu (chỉ hỗ trợ email, đã bỏ xác thực SMS)
async function forgotPasswordVerify(req, res) {
  try {
    ...
    if (!newPassword) {
      if (method !== 'email') {
        return res.status(400).json({ message: 'Only email verification is supported. SMS is disabled.' });
      }

      if (!identifier || !identifier.endsWith('@gmail.com')) {
        return res.status(400).json({ message: 'Email must be @gmail.com' });
      }
      ...
      // Kiểm tra OTP (chỉ email)
      const purpose = 'forgot_password_email';
    ...
    // Nếu có newPassword thì verify OTP + đổi mật khẩu (chỉ email)
    if (method !== 'email') {
      return res.status(400).json({ message: 'Only email verification is supported. SMS is disabled.' });
    }
